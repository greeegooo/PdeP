...
datosTest6 1 = ("Si Rodri se rescata 5 horas queda con 255 de resistencia", resistencia (rescatarse 5 rodri) == 255, True)
datosTest6 2 = ("Si Rodri se rescata 1 hora queda con 155 de resistencia", resistencia (rescatarse 1 rodri) == 155, True)
...
datosEntrega2Test1c 1 = ("Rodri toma una soda de nivel 1 y una soda de nivel 2 y queda con nombre errperpRodri", (nombre . soda 2 . soda 1) rodri == "errperpRodri", True)
...
datosEntrega2Test2b 1 = ("Rodri puede tomar dos bebidas, entre un grog XD, un tintico y un klusener de frutilla", cuantasPuedeTomar rodri [grogXD, tintico, klusener "frutilla"] == 2, True)
...
datosEntrega2Test3b 1 = ("Rodri hace una salida de amigos... y le pasa de todo", nombre (tomarTragos rodri (bebidas salidaDeAmigos)) == "erpRodri" && (length . tomadas) (tomarTragos rodri (bebidas salidaDeAmigos)) == 4 && (length . amigos) (tomarTragos rodri (bebidas salidaDeAmigos)) == 1 && (resistencia) (tomarTragos rodri (bebidas salidaDeAmigos)) == 45 , True)
...
datosEntrega2Test4b' 1 = ("Rodri hace el itinerario más intenso entre una salida de amigos, la mezcla explosiva y el itinerario básico y realiza salida de amigos", "erpRodri" == nombre (tomarElMasIntenso rodri [salidaDeAmigos,mezclaExplosiva,itinerarioBasico]) && (length . tomadas) (tomarElMasIntenso rodri [salidaDeAmigos,mezclaExplosiva,itinerarioBasico]) == 4 && (length . amigos) (tomarElMasIntenso rodri [salidaDeAmigos,mezclaExplosiva,itinerarioBasico]) == 1 && resistencia (tomarElMasIntenso rodri [salidaDeAmigos,mezclaExplosiva,itinerarioBasico]) == 45, True)
